 ...
 * Classic Editor.
 *
 * Enables the WordPress classic editor and the old-style Edit Post screen
 * with TinyMCE, Meta Boxes, etc. Supports the older plugins
 * that extend this screen.
 *
 * @package Classic_Editor
 *
...
if ( ! class_exists( 'Classic_Editor' ) ) :
	 ...
	 *
	 * Provides functionality to enable the classic WordPress editor
	 * and replace the block editor when desired.
	 *
	...
	final class Classic_Editor {
		 ...
		 * Constructor.
		 *
		 * This class should only be instantiated statically.
		 */
		 ...
		 *
		 * Sets up all WordPress hooks and filters needed by the plugin.
		 *
		 * @return void
		 */
		public static function init_actions(): void {
			...
			if ( is_multisite() ) {
				add_action( 'wpmu_options', array( self::class, 'network_settings' ) );
				add_action( 'update_wpmu_options', array( self::class, 'save_network_settings' ) );
			}

			if ( ! self::$settings['hide-settings-ui'] ) {
				add_filter( 'plugin_action_links', array( self::class, 'add_settings_link' ), 10, 2 );
				add_filter( 'network_admin_plugin_action_links', array( self::class, 'add_settings_link' ), 10, 2 );
				add_action( 'admin_init', array( self::class, 'register_settings' ) );
				if ( self::$settings['allow-users'] ) {
					add_action( 'personal_options_update', array( self::class, 'save_user_settings' ) );
					add_action( 'edit_user_profile_update', array( self::class, 'save_user_settings' ) );
					add_action( 'profile_personal_options', array( self::class, 'user_settings' ) );
					add_action( 'edit_user_profile', array( self::class, 'user_settings' ) );
				}
			}

			remove_action( 'try_gutenberg_panel', 'wp_try_gutenberg_panel' );
			add_action( 'admin_print_styles', array( self::class, 'safari_fix' ) );
			global $wp_version;
			if ( version_compare( $wp_version, '6.7.1', '>=' ) && version_compare( $wp_version, '6.8', '<' ) && is_admin() ) {
				add_filter( 'script_loader_src', array( self::class, 'fix_post_js' ), 11, 2 );
			}
			...
			if ( self::$settings['allow-users'] ) {
				add_filter( 'use_block_editor_for_post', array( self::class, 'choose_editor' ), 100, 2 );
				if ( $is_gutenberg ) {
					add_filter( 'gutenberg_can_edit_post', array( self::class, 'choose_editor' ), 100, 2 );
					if ( 'classic' === self::$settings['editor'] ) {
						self::remove_gutenberg_hooks( 'some' );
					}
				}

				add_filter( 'get_edit_post_link', array( self::class, 'get_edit_post_link' ) );
				add_filter( 'redirect_post_location', array( self::class, 'redirect_location' ) );
				add_action( 'edit_form_top', array( self::class, 'add_redirect_helper' ) );
				add_action( 'admin_head-edit.php', array( self::class, 'add_edit_php_inline_style' ) );
				add_action( 'edit_form_top', array( self::class, 'remember_classic_editor' ) );
				if ( version_compare( $wp_version, '5.8', '>=' ) ) {
					add_filter( 'block_editor_settings_all', array( self::class, 'remember_block_editor' ), 10, 2 );
				} else {
					add_filter( 'block_editor_settings', array( self::class, 'remember_block_editor' ), 10, 2 );
				}

				add_filter( 'display_post_states', array( self::class, 'add_post_state' ), 10, 2 );
				add_filter( 'page_row_actions', array( self::class, 'add_edit_links' ), 15, 2 );
				add_filter( 'post_row_actions', array( self::class, 'add_edit_links' ), 15, 2 );
				add_action( 'add_meta_boxes', array( self::class, 'add_meta_box' ), 10, 2 );
				add_action( 'add_meta_boxes', array( self::class, 'add_meta_box_bidirectional' ), 10, 2 );
				add_action( 'enqueue_block_editor_assets', array( self::class, 'enqueue_block_editor_scripts' ) );
				add_action( 'wp_ajax_classic-editor-switch', array( self::class, 'ajax_switch_editor' ) );
			...
			if ( $block_editor ) {
				add_action( 'admin_init', array( self::class, 'on_admin_init' ) );
			}
		...
		public static function register_rest_api(): void {
			register_rest_route(
				...
				array(
					...
					'callback'            => array( self::class, 'rest_update_preferences' ),
			...
			register_rest_route(
				...
				array(
					...
					'callback'            => array( self::class, 'rest_get_preferences' ),
					'permission_callback' => static fn() => current_user_can( 'edit_posts' ),
				)
			);
		}
		...
		public static function enqueue_frontend_scripts(): void {
			...
			wp_register_script( 'classic-editor-switcher', plugins_url( 'js/switcher.js', __FILE__ ), array(), '1.7.0', array( 'in_footer' => true ) );
			wp_localize_script(
